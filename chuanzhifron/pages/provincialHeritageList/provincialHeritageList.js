...
Page({
  ...
  loadHeritageData: function() {
    request({
      ...
      // 只获取省级项目（level === 2）
      let provincialList = processedData.filter(item => item.level === 2);
      
      // 根据分类筛选
      if (this.data.currentCategory !== 'all') {
        provincialList = provincialList.filter(item => item.category === this.data.currentCategory);
      }
      
      this.setData({
        heritageList: provincialList,
        loading: false
      });
    }).catch(err => {
